...
Гейт покрытия (Stop-хук final_gate.sh): жёсткие 85%. На время разработки без
тестов порог можно временно ослабить файлом `.coverage-grace` в корне (число-порог,
`#` — комментарии); файл удалить по завершении — 85% вернутся сами.
